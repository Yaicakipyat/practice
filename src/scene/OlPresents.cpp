#include <iostream>
#include <algorithm>
#include <cstdlib>      // Добавлено для rand, srand
...
ol::OlPresents::OlPresents(SceneManager& sceneManager) : Scene(sceneManager)
{
    ...
    for (int i = 0; i < OL_PRESENTS_TEXT.size(); i++)
    {
        ...
        letter.path.addPoint(Vec2(i * 8 + 60.0f, 110.0f));
        letters_.push_back(letter);
    }
    
    // Ручная реализация random_shuffle для совместимости с C++17
    ...
    for (size_t i = letters_.size() - 1; i > 0; --i) {
        size_t j = std::rand() % (i + 1);
        std::swap(letters_[i], letters_[j]);
    }
...
void ol::OlPresents::update()
{
    ...
    {
        if  (getGameElapsedTime() - lettersFinishedTime_ > 10.0f)
        {
            sceneManager_.changeScene(Scene::Id::TITLE);
        }
        else if  (getGameElapsedTime() - lettersFinishedTime_ > 9.0f)
        {
