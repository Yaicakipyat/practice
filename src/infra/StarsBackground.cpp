...
void ol::StarsBackground::createStars()
{
    ...
    for (int v = 0; v < 3; v++)
    {
        for (int i = 0; i < STARS_COUNT / 3; i++)
        {
            ...
            stars_[index].position.x = ol::random(0, Display::SCREEN_WIDTH);

            stars_[index].position.y = ol::random(0, Display::SCREEN_HEIGHT) -
                                       2 * Display::SCREEN_HEIGHT;
...
void ol::StarsBackground::update()
{
    ...
    for (int i = 0; i < STARS_COUNT; i++)
    {
        ...
        if (stars_[i].position.y > ol::Display::SCREEN_HEIGHT)
        {
